...
class IpGeolocationDotIoMapper implements Mapper
{
    public function map(RequestResponseWrapper $rawIpResults): NormalizedGeolocationResultsData
    {
        return new NormalizedGeolocationResultsData(
            ...
            optional($rawIpResults->data['organization'] ?? null, static fn (string $organization) => blank($organization) ? null : $organization),
        );
    }

    public function supports(string $request): bool
    {
        return $request === IpGeolocationDotIoDriver::class;
    }
}
